<div x-data="{
    ...
    <button wire:click="toggleChat"
        ...
        <div class="relative"><span class="w-2 h-2 bg-green-500 rounded-full block"></span><span
                class="absolute inset-0 bg-green-500 rounded-full animate-ping"></span></div>
    ...
    @if ($isOpen)
        <div x-transition
            ...
            <div class="px-6 py-5 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                ...
                <button wire:click="toggleChat" class="text-white/20 hover:text-white"><svg class="w-6 h-6"
                    ...
                    <textarea wire:model="userInput" @keydown.enter.prevent.exact="$wire.sendMessage()"
                        ...
                        rows="1" x-data="{ resize() { $el.style.height = '50px';
                                $el.style.height = $el.scrollHeight + 'px' } }" x-init="$nextTick(() => { $el.focus();
                                    resize(); })" @input="resize()" autofocus></textarea>
